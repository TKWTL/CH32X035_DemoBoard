/********************************** (C) COPYRIGHT *******************************
 * Runtime PIOC API used by the board-level WS2812 driver.
 *
 * The embedded instruction image is still the validated WCH RGB1W/DS1W image;
 * only its IO0 + SFR RGB command is exposed by this API. CH32X035 maps the PIOC
 * program store onto a fixed 4 KiB SRAM window while PIOC runs, so shortening
 * the image cannot return that window to the CPU. A future RGB-only reassembly
 * may reduce Flash/copy size, but Link.ld must still reserve the top 4 KiB.
 *******************************************************************************/
...
static const unsigned char PIOC_WS2812_CODE[] __attribute__((aligned(4))) =
...
static void PIOC_API_SendRaw(uint16_t length, const uint8_t *data)
{
    if((data == NULL) || (length == 0u) || (length > PIOC_API_SFR_SIZE))
        return;

    /* Reset/halt PIOC, then run IO0 on the board DIN alias. */
    PIOC->D8_SYS_CFG = RB_MST_RESET | RB_MST_IO_EN0;
    PIOC->D8_SYS_CFG = RB_MST_CLK_GATE | RB_MST_IO_EN0;

    memcpy(PIOC_API_SFR_ADDR, data, length);
    PIOC_API_COMMAND = (uint8_t)length;
}
...
void PIOC_API_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO | DIN_GPIO_CLK, ENABLE);
    ...
    GPIO_Init(DIN_GPIO_Port, &gpio);

    PIOC->D8_SYS_CFG = RB_MST_RESET | RB_MST_IO_EN0;
    memcpy((uint8_t *)PIOC_SRAM_BASE, PIOC_WS2812_CODE, sizeof(PIOC_WS2812_CODE));

#if defined SYSCLK_FREQ_24MHz_HSI
    *(uint32_t *)(PIOC_SRAM_BASE + PIOC_FREQ_CFG) = 0x00300030u;
#endif
}
...
uint8_t PIOC_API_Send(const uint8_t *data, uint16_t length)
{
    ...
    PIOC_API_SendRaw(length, data);
    start_us = TIME_Micros();

    while((PIOC->D8_SYS_CFG & RB_INT_REQ) == 0u)
    {
        if((uint32_t)(TIME_Micros() - start_us) >= PIOC_API_WAIT_TIMEOUT_US)
        {
            PIOC->D8_SYS_CFG = RB_MST_RESET | RB_MST_IO_EN0;
            return PIOC_API_ERR_TIMEOUT;
        }
    }

    /* Reading CTRL_RD clears the PIOC request flag. */
