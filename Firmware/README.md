...
- SPR 协商最高至 20 V；当供电源（Source）支持时，进入 PD 3.1 EPR 并请求最高可用的固定 EPR 电压（默认上限 28 V；升级到 36 V 挡位 = 将 EPR 上限宏调至 36000 + 硬件将 D2 SMAJ28A 更换为 SMAJ36A，见仓库根 README「更新说明」）。
...
`Peripheral/PD/pd.c` 负责 Sink 协议/策略状态：SPR/EPR 固定挡位选择（EPR 上限宏默认 28 V；36 V 挡位需同时将 D2 SMAJ28A 更换为 SMAJ36A）、自适应 Enter PDP 的 EPR 进入、KeepAlive、超时/恢复与合同状态。它不直接接触 CH32X035 USBPD 寄存器。
